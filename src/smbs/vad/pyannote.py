"""Pyannote VAD pipeline — GPU-based voice activity detection."""
...
from smbs.config import METADATA_DIR
...
def log_progress(i, total, start_time, recent_times):
    ...
    print(f"[{i:>6}/{total}] {rolling_rate:.1f} files/s | {eta_str}")


def get_log_interval(files_processed):
...
def compute_segment_stats(durations):
    if not durations:
...
def compute_nospch_durations(speech_segments, duration):
    if not speech_segments:
        return [duration]

    gaps = []
    if speech_segments[0][0] > 0.001:
...
def create_error_record(path, error):
    return {
        "success": False,
        "path": path,
        ...
        "speech_durations": [], "nospch_durations": [],
        "speech_segments": [], "speech_segments_detailed": [],
        "error": error,
    }


def process_single_file(audio_dict, pipeline):
    ...
    for segment in speech_segments:
        ...
        speech_segments_detailed.append({
            "start_sec": start_sec, "start_frames": start_frames,
            "end_sec": end_sec, "end_frames": end_frames,
            "duration_sec": dur_sec, "duration_frames": int(dur_sec * sample_rate),
        })
...
def process_files(dataloader, pipeline, metadata_path, task_id, chunk_size):
    results = []
    errors = 0
    fragment_id = 0
...
def merge_fragments(fragment_dir, final_path):
...
def run_pyannote(manifest: str, array_id: int = 0, array_count: int = 1) -> None:
    """Run pyannote VAD on a manifest."""
    print(f"Task {array_id}/{array_count - 1} starting")
    ...
    if hf_token is None:
        raise RuntimeError(
            "HF_TOKEN not found. Set it in .env or: export HF_TOKEN='your_token_here'"
        )

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    pipeline = setup_model_and_pipeline(hf_token, device)

    total_files, chunk_size, paths = get_task_shard(manifest, array_id, array_count)
    ...
    metadata_path = METADATA_DIR / Path(manifest).stem / "pyannote"
    metadata_path.mkdir(parents=True, exist_ok=True)

    errors = process_files(dataloader, pipeline, metadata_path, array_id, chunk_size)

    if errors > 0:
        print(f"WARNING: {errors} errors encountered", file=sys.stderr)
    print(f"Task {array_id} complete")
